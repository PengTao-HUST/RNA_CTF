...
! derived from them that are needed during mdin &ctrl printout, etc.
  ...
  integer                       imin, nmropt, ntx, irest, ntrx, ntxo, &
                                ...
                                mcrescyc,mcverbosity, &
                                infe, & ! added by FENG PAN
                                ineb, skmin, skmax, tmode, vv, &
                                irandom, mask_from_ref, mbar_states, &
                                geq_nstep, ginit_vel, gsyn_mass, & !GTI
                                gremd_acyc, gti_cpu_output, & ! GTI
                                midpoint,&
  ...
  common / mdin_ctrl_int /      imin, nmropt, ntx, irest, ntrx, ntxo, &     !  6
                                ...
                                ntcmdprep,ntebprep,efn,infe,irandom, &  ! 91
                                icnste,ntcnste,ntrelaxe,             &  ! 94
                                mask_from_ref, mbar_states,           &   ! 96
                                geq_nstep, ginit_vel,  gsyn_mass, & !99
                                gremd_acyc, gti_cpu_output, & !101
                                mcint,mcrescyc,mcverbosity, & !104
                                ineb,skmin,skmax,tmode,vv,  & !109
                                iphmd, midpoint, &!111
                                !--------- cotranslational folding -----------
                                itunnel,staysteps!113 
                                !---------------------------------------------

  !integer, parameter    :: mdin_ctrl_int_cnt = 111
  !--------- cotranslational folding -----------
  ...
  double precision              dielc, es_cutoff, vdw_cutoff, fswitch, &
                                ...
                                sigma0D,sigma0P,sigmaVD,sigmaVP,cutoffD,cutoffP, &
                                ...
                                !--------- cotranslational folding -----------
                                tunnellength,cylinderradius,conemaxradius,&
                                coneheight,xminpos,ctdy,ctdz 
                                !---------------------------------------------
                                

  real, dimension(100) :: mbar_lambda
  ...
  common / mdin_ctrl_dbl /      dielc, es_cutoff, vdw_cutoff, &                  !3
                                ...
                                gammamap,                                    &   !83
                                gb_alpha_hnu, gb_beta_hnu, gb_gamma_hnu, &       !86
                                gb_alpha_cnu, gb_beta_cnu, gb_gamma_cnu, &       !89
                                gb_alpha_nnu, gb_beta_nnu, gb_gamma_nnu, &       !92
                                gb_alpha_osnu, gb_beta_osnu, gb_gamma_osnu, &    !95
                                gb_alpha_pnu, gb_beta_pnu, gb_gamma_pnu, &       !98
                                screen_hnu, screen_cnu, screen_nnu, &            !101
                                screen_onu, screen_pnu,  &                       !103
                                VmaxD,VminD,VavgD,k0D,VmaxP,VminP,VavgP,k0P,kD,kP, & ! 113
                                sigma0D,sigma0P,sigmaVD,sigmaVP,cutoffD,cutoffP, fswitch,& ! 120
                                efx,efy,efz,efphase,effreq,solve,mbar_lambda,mcwatmaxdif,& !128
                                vfac,mcboxshift,&!130
                                !--------- cotranslational folding -----------
                                tunnellength,cylinderradius,conemaxradius,&
                                coneheight,xminpos,ctdy,ctdz!137
                                !---------------------------------------------                                

  !integer, parameter    :: mdin_ctrl_dbl_cnt = 131 ! This number needs to be the count ended on the previous line plus 1
  !--------- cotranslational folding -----------
  integer, parameter    :: mdin_ctrl_dbl_cnt = 138
  !---------------------------------------------
  save  :: / mdin_ctrl_dbl /
...
!DG: removed the private type from tgtfitmask and tgtrmsmask
  character(256)       :: tgtrmsmask, tgtfitmask, scmask
  ...
  namelist /cntrl/      imin, nmropt, ntx, irest, ntrx, ntxo, &
                        ...
                        igamd,irest_gamd,iE,iVm,igamdlag,kD,kP,ntcmd,nteb,ntcmdprep,ntebprep, &
                        ...
                        geq_nstep, ginit_vel, gsyn_mass, gremd_acyc, &
                        gti_cpu_output,mcwat,mcint,mcrescyc,mcverbosity,&
                        mcresstr,mcwatmaxdif,ineb,skmin,skmax,tmode,vv,vfac,&
                        mcboxshift, midpoint,&
                        !--------- cotranslational folding -----------
                        itunnel,staysteps,tunnellength,cylinderradius,conemaxradius,&
                        coneheight,xminpos,ctdy,ctdz
                        !---------------------------------------------
...
!External Electric Field
  ...
  mcint=0               ! mcint - interval for mcwat monte carlo moves.
  mcrescyc=1            ! mcrescyc - controls number of mc moves per mcint - this is max attempts. If t gets
                        !            an acceptance in less than mcrescyc it will break the loop and use that
                        !            acceptance.
...
! GaMD
  if (igamd.gt.0) then
    ...
    if (irest_gamd.eq.1) then
      ...
      ntcmd = 0
      ntcmdprep = 0
      write(mdout,'(a,2i10)')'| GaMD restart simulation; set (ntcmd,ntcmdprep) = ', ntcmd, ntcmdprep
    end if
  end if
! end GaMD
  ...
  !---------------- cotranslational folding -------------------
  if (itunnel .ne. 0 .and. itunnel .ne. 1) then
    write(mdout, '(a,a)') error_hdr, 'itunnel must be set to 0 or 1!'
    inerr = 1
  end if

  if (itunnel .eq. 1) then

    if (staysteps .lt. 0) then
      write(mdout, '(a,a)') error_hdr, 'staysteps must be > 0!'
      inerr = 1
    end if
    
    if (tunnellength .lt. 0) then
      write(mdout, '(a,a)') error_hdr, 'tunnellength must be > 0!'
      inerr = 1
    end if

    if (cylinderradius .lt. 0) then
      write(mdout, '(a,a)') error_hdr, 'cylinderradius must be > 0!'
      inerr = 1
    end if
    
    if (conemaxradius .lt. 0) then
      write(mdout, '(a,a)') error_hdr, 'conemaxradius must be > 0!'
      inerr = 1
    end if
    
    if (coneheight .lt. 0) then
      write(mdout, '(a,a)') error_hdr, 'coneheight must be > 0!'
      inerr = 1
    end if

    if (abs(xminpos-0.0) .lt. 0.001) then
      write(mdout, '(a,a)') error_hdr, 'xminpos is not set yet!'
      inerr = 1
    end if

    if (abs(ctdy-0.0) .lt. 0.001) then
      write(mdout, '(a,a)') error_hdr, 'ctdy is not set yet!'
      inerr = 1
    end if

    if (abs(ctdz-0.0) .lt. 0.001) then
      write(mdout, '(a,a)') error_hdr, 'ctdz is not set yet!'
      inerr = 1
    end if
...
!DG: changed this to allow the use of tgtrmsmask and tgtfitmask in  neb
  ...
  if (icnste .eq. 1 .and. using_pme_potential) then
    write(mdout, '(2a)') error_hdr, 'use icnste=2 for explicit constant pH MD!'
    inerr = 1
  end if

  if (icnste .eq. 2 .and. .not. using_pme_potential) then
    write(mdout, '(2a)') error_hdr, 'use icnste=1 for implicit constant pH MD!'
    inerr = 1
  end if
...
!igamd=0 no boost is used, 1 boost on the total energy, 2 boost on the dohedrals, 3 boost on dihedrals and total energy
  if(igamd.gt.0)then
    ...
    if(igamd.eq.1 .or. igamd.eq.4)then !only total/non-bonded potential energy will be boosted
      EthreshD=0.0
      kD=0.0
    endif
    ...
    if (master) then
      ...
      write(mdout,'(a,3i3)')     '| igamd,iE         = ',igamd,iE
      write(mdout,'(a,5I10)')    '| ntcmd,nteb,ntave = ', ntcmd,nteb,ntave
      write(mdout,'(a,2I10)')    '| ntcmdprep,ntebprep  = ', ntcmdprep,ntebprep
      write(mdout,'(a,4f14.4)') '| sigma0P,sigma0D  = ', sigma0P,sigma0D
...
! Thermodynamic Integration validation
  if (icfe .eq. 0) then
    ...
    if (igamd .ne. 0) then
      write(mdout, '(a,a)') error_hdr, 'TI is incompatible with GaMD'
      inerr = 1
    end if
  ...
  if (ifsc .ne. 0) then
    ...
    if(ifmbar .ne. 0) then
      ...
      if (mbar_lambda(1) .eq. 2) then
        write(mdout, '(a,a)') error_hdr, 'mbar_lambda not set, please set &
          & mbar_lambda to the lambdas you wish to sample'
        inerr = 1
      end if
...
subroutine print_mdin_ctrl_dat(remd_method, cph_igb, ce_igb)
  ...
  if (igb .eq. 1 .or. igb .eq. 2 .or. igb .eq. 5 .or. igb .eq. 7 .or. &
      ...
      ce_igb .eq. 5 .or. ce_igb .eq. 7) then
  ...
  else if (igb .eq. 8 .or. cph_igb .eq. 8 .or. ce_igb .eq. 8) then
  ...
  !------------------- cotranslational folding ----------------------
  write(mdout,'(/a)') 'Cotranslational folding:'
  
  write(mdout,'(5x,a,i8)') 'itunnel        =', itunnel
  if (itunnel==1) then
    write(mdout,'(5x,a,i8,a,f8.2)') 'staysteps      =', staysteps,&
          ', xminpos        =',xminpos
    write(mdout,'(5x,2(a,f8.2))') 'tunnellength   =', tunnellength,&
          ', cylinderradius =', cylinderradius
    write(mdout,'(5x,2(a,f8.2))') 'conemaxradius  =',conemaxradius, &
          ', coneheight     =',coneheight
    write(mdout,'(5x,2(a,f8.2))') 'ctdy           =',ctdy,&
          ', ctdz           =',ctdz
  end if
  !-----------------------------------------------------------------
  if (imin .ne. 0) then
...
! Constant Redox potential information
  if (icnste .ne. 0) then
    ...
    write(mdout, '(5xa,f10.5)') 'solve =', solve
    if (icnste .ne. 1) &
...
! Local variables
  ...
  if (iE.eq.1) then
    ...
    k0_2 = (1.0-sigma0/sigmaV)*(Vmax-Vmin)/(Vavg-Vmin)
    if ( (k0_2.gt.0) .and. (k0_2.le.1) ) then
      k0 = k0_2
      E = Vmin + (Vmax - Vmin)/k0
    else
      E = Vmax
